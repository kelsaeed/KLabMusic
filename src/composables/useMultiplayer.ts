...
let unsubscribeArrangeAction: (() => void) | null = null
...
export function useMultiplayer() {
  ...
  async function joinRoom(code: string, name: string): Promise<{ ok: boolean; message?: string }> {
    ...
    channel
      ...
      .on('broadcast', { event: 'arrange:state' }, ({ payload }) => {
        ...
        // Also serves as the divergence-recovery backstop when an
        // arrange:patch packet is dropped or reordered.
        const p = payload as { state: ArrangeSharedState; player: string }
        if (p.player === store.localId) return
        applyArrangeState(p.state)
      })
      .on('broadcast', { event: 'arrange:patch' }, ({ payload }) => {
        // Per-action arrangement patch — clip drags, automation drags,
        // FX knob drags, track volume sweeps. Sized in tens of bytes vs
        // the kilobytes of a full track snapshot, so a 60Hz drag on the
        // timeline doesn't saturate the realtime channel. We replay the
        // action through the same store API the local user calls so
        // both peers end up with byte-identical state.
        const p = payload as
          | { op: 'moveClip'; trackId: string; clipId: string; newStartSec: number; player: string }
          | { op: 'addAutomationPoint'; trackId: string; param: ArrangeTrack['automationParam']; point: { time: number; value: number }; player: string }
          | { op: 'moveAutomationPoint'; trackId: string; param: ArrangeTrack['automationParam']; pointIndex: number; next: { time: number; value: number }; player: string }
          | { op: 'removeAutomationPoint'; trackId: string; param: ArrangeTrack['automationParam']; pointIndex: number; player: string }
          | { op: 'setTrackVolume'; trackId: string; volume: number; player: string }
          | { op: 'updateTrackFx'; trackId: string; key: keyof ArrangeTrack['fx']; patch: { enabled?: boolean; amount?: number }; player: string }
        if (p.player === store.localId) return
        // holdReceiving — same reasoning as beat:patch: prevents the
        // store action from triggering a 200ms full-state echo back to
        // the sender once the receiving flag clears.
        holdReceiving()
        if (p.op === 'moveClip') {
          arrangeStore.moveClip(p.trackId, p.clipId, p.newStartSec)
        } else if (p.op === 'addAutomationPoint') {
          arrangeStore.addAutomationPoint(p.trackId, p.param, p.point)
        } else if (p.op === 'moveAutomationPoint') {
          arrangeStore.moveAutomationPoint(p.trackId, p.param, p.pointIndex, p.next)
        } else if (p.op === 'removeAutomationPoint') {
          arrangeStore.removeAutomationPoint(p.trackId, p.param, p.pointIndex)
        } else if (p.op === 'setTrackVolume') {
          arrangeStore.updateTrack(p.trackId, { volume: p.volume })
        } else if (p.op === 'updateTrackFx') {
          arrangeStore.updateTrackFx(p.trackId, p.key, p.patch)
        }
      })
  ...
  function wireSyncSubscriptions() {
    ...
    // Arrange-side patches — same shape as beat:patch but for the
    // arrangement timeline's bandwidth-heavy gestures: clip drag,
    // automation curve drag, FX knob drag, track volume slider.
    // Add/remove track + add/remove clip stay on full-state because the
    // receiver needs the post-mutation snapshot anyway (track + clip
    // ids are generated by the sender and must round-trip verbatim).
    unsubscribeArrangeAction = arrangeStore.$onAction(({ name, args, after }) => {
      after(() => {
        if (!store.isConnected || isReceiving.value) return
        if (name === 'moveClip') {
          const [trackId, clipId, newStartSec] = args as [string, string, number]
          broadcast('arrange:patch', {
            op: 'moveClip',
            trackId,
            clipId,
            newStartSec,
        ...
        } else if (name === 'addAutomationPoint') {
          const [trackId, param, point] = args as [
            string,
            ArrangeTrack['automationParam'],
            { time: number; value: number },
          ]
          broadcast('arrange:patch', {
            op: 'addAutomationPoint',
            trackId,
            param,
            point,
            player: store.localId,
          })
        } else if (name === 'moveAutomationPoint') {
          const [trackId, param, pointIndex, next] = args as [
            string,
            ArrangeTrack['automationParam'],
            number,
            { time: number; value: number },
        ...
        } else if (name === 'removeAutomationPoint') {
          const [trackId, param, pointIndex] = args as [
            string,
            ArrangeTrack['automationParam'],
            number,
          ]
        ...
        } else if (name === 'updateTrack') {
          // updateTrack is a generic patch object; restrict the patched
          // fields we replicate to the bandwidth-heavy ones (volume) so
          // a knob drag broadcasts ~30 bytes instead of the whole
          // ArrangeTrack object. Other fields fall through to the
          // 200ms full-state path, which still reaches the receiver.
          const [trackId, patch] = args as [string, Partial<ArrangeTrack>]
        ...
        } else if (name === 'updateTrackFx') {
          const [trackId, key, patch] = args as [
            string,
            keyof ArrangeTrack['fx'],
  ...
  function unwireSyncSubscriptions() {
    ...
    if (unsubscribeArrangeAction) { unsubscribeArrangeAction(); unsubscribeArrangeAction = null }
  }
